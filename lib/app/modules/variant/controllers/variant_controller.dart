...
class VariantController extends GetxController {
  VariantController({required int id}) {
    _id = id.obs;
  }
  late VariantClient client;
  final stateScreen = StateModel<VariantResponse>().obs;
  ...
  void _data(VariantResponse data) {
  ...
  void _moreData(VariantResponse data) {
    ...
    stateScreen().maybeWhen(
      ...
      data: (metadata) async {
        if (page < metadata.totalPages) {
          page.value++;
          try {
            final data = await client.getVariants(
                limit: limit, page: page(), brandId: _id(), title: '');
            _moreData(data);
  ...
  Future<RefreshStatus> fetchVariants({
    int limit = 10,
  }) async {
    page(1);
